<template>
  <div>
    <div class="form" v-if="!sended"> 
      <input type="text" v-model="surname" placeholder="Фамилия" :class="{'input-error': !surnameValidate}" @input="validateSurname">
      <p v-if="!surnameValidate">Это поле не должно быть пустым</p>
      <input type="text" v-model="name" placeholder="Имя" :class="{'input-error': !nameValidate}" @input="validateName">
      ...
      <input type="text" v-model="phone" placeholder="Телефон" :class="{'input-error': !phoneValidate}" @input="validatePhone">
      <p v-if="!phoneValidate">Номер телефона должен состоять из 11 цифр</p>
      <input type="text" v-model="email" placeholder="E-mail" :class="{'input-error': !emailValidate}" @input="validateEmail">
      ...
      <input type="password" v-model="passConfirm" placeholder="Подтвердите пароль" :class="{'input-error': !passValidateConfirm}" @input="validateConfirm">
...
export default {  
  ...
  methods: {
    ...
    send: async function() {
      ...
      if (!this.surnameValidate || !this.nameValidate || !this.emailValidate || !this.phoneValidate || !this.passValidate || !this.passValidateConfirm) {
      ...
      .then(response => {
          ...
          if (response.data.result) {
            ...
            this.result = false;
            if (response.data.errMess.includes("phone_unique"))
              this.mess = 'Такой номер телефона уже зарегистрирован в системе';
            else if (response.data.errMess.includes("email_unique"))
